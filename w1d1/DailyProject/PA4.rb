...
def price_is_right(bids, actual_retail_price)
  bids.select! { |price| price < actual_retail_price }
  bids.min_by { |price| actual_retail_price - price }
end
...
def one_week_wonders(songs)
  ...
  songs.each_with_index do |song, idx|
    (song != songs[idx + 1]) ? arr << song : nope << song
  end
  (arr - nope).uniq
end
