...
def greatest_common_factor(first_number, second_number)
  arr = (1..first_number).to_a
  arr.select! do |num|
    num if first_number % num == 0 && second_number % num == 0
  end
  arr.last
end
...
def panoramic_pairs(landmarks)
  new_arr = []
  arr = landmarks + [landmarks.first]
  arr.each_with_index do |land1, idx|
    break if idx == arr.length - 1
    new_arr << [land1, arr[idx + 1]]
  end
  new_arr
end
...
def two_degrees_away(facebook, name)
  arr = []
  persons_friends = facebook[name] # an array
  persons_friends.each do |person|
    arr << facebook[person]
  end
  arr.flatten!
  arr.select { |i| i != name && !persons_friends.include?(i)}
end
...
def assign_pods(students, pods)
  pod_hash = Hash.new()
  pods.each { |i| pod_hash[i] = [] }
  pod_hash.each do |keys, value|
    students.each do |ppl|
      next if pod_hash.values.flatten.any? {|i| i == ppl}
      pod_hash[keys] << ppl
      break if pod_hash[keys].length == 4
    end
  end
  pod_hash
end
