...
def in_all_strings?(long_strings, short_string)
  long_strings.all? { |word| word.length != word.gsub(short_string,"").length}
end
...
def biodiversity_index(specimens)
  species_arr = specimens.uniq
  num_species = species_arr.length
  smallest_pop = specimens.length
  largest_pop = 0
  species_arr.each do |ani|
    pop = specimens.count(ani)
    smallest_pop = pop if pop < smallest_pop
    largest_pop = pop if pop > largest_pop
  end
...
def for_fs_sake(string)
  words = string.split(" ")
  p words
  x = words.map { |el| (el.split("").rindex("f").class == Fixnum) ? el.split("").rindex("f") : string.length }
  return "" if x.all? { |el| el == string.length}
  words[x.index(x.min)]
end
