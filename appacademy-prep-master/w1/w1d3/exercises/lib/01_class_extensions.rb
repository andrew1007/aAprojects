...
class String
  def caesar(shift)
    ...
    shifted_lower = lower_alpha[shift..-1] + lower_alpha[0...shift]
    shifted_capital = capital_alpha[shift..-1] + capital_alpha[0...shift]
    alpha = lower_alpha + capital_alpha
    shifted = shifted_lower + shifted_capital
    cipher = Hash[alpha.zip(shifted)]
    splt = self.split("")
    mapped = splt.map {|i| cipher[i]}
    return mapped.join("")
  end
end
...
class Hash
  def difference(other_hash)
    merged = self.merge(other_hash)
    new_hash = {}
    self.each do |key, value|
      new_hash[key] = value if !other_hash.keys.include?(key)
    end
    other_hash.each do |key, value|
      new_hash[key] = value if !self.keys.include?(key)
    end
    return new_hash
  end
end
...
class Fixnum
  def stringify(base)
    base_list = (0..15).to_a
    map_value = (0..9).to_a + ("a".."f").to_a
    hash_conversion = Hash[base_list.zip(map_value)]
    i = 1
    powers = [1]
    until base**i > self
      powers << base**i
    ...
    conversion = powers.map {|power| (self / power)%base}
    new_symbols = conversion.map {|key| hash_conversion[key]}
    return new_symbols.reverse.join("")
  end
end
