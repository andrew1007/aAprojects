...
def lrt_stops
  ...
  execute(<<-SQL)
    SELECT id, name
    FROM stops
      INNER JOIN routes ON stops.id = stop_id
...
def haymarket_and_leith
  ...
  execute(<<-SQL)
    ...
    FROM routes r1
      JOIN routes r2 ON r1.num = r2.num
    WHERE r1.stop_id = 115 AND r2.stop_id = 137
...
def craiglockhart_to_sighthill
  ...
  FROM routes r1
    JOIN routes r2 ON r1.num = r2.num AND r1.company = r2.company
    JOIN stops s1 ON s1.id = r1.stop_id
    JOIN stops s2 ON s2.id = r2.stop_id
    JOIN routes r3 ON r3.stop_id = s2.id
    JOIN stops s3 ON s3.id = r3.stop_id
    JOIN routes r4 ON r3.num = r4.num AND r3.company = r4.company
    JOIN stops s4 ON r4.stop_id = s4.id
  WHERE
    s1.name = 'Craiglockhart' AND s4.name = 'Sighthill'
  SQL
end
