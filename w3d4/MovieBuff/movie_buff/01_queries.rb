...
def harrison_ford
  ...
  Movie.joins(:actors).where("actors.name = 'Harrison Ford' AND NOT castings.ord = 1")
...
def biggest_cast
  ...
  Movie.select("movies.id, movies.title").group("movies.id").order("COUNT(*) DESC").joins(:castings).limit(3)
end
